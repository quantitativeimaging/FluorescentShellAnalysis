function tiled_segments = tile_segments(shell_segments, num_segments)

num_rows = floor(sqrt(num_segments));
num_cols = ceil(num_segments / num_rows);
segment_size = size(shell_segments, 2);
...
for i=1:(num_segments)
	...
	tiled_segments(rows, cols) = shell_segments((1 + (i - 1) * segment_size):(i * segment_size), 1:segment_size);
end
